...
func ripTrack(track *task.Track, token string, mediaUserToken string) {
	...
	mp4boxExec := getMP4BoxExec()
	cmd := exec.Command(mp4boxExec, "-itags", tagsString, trackPath)
	if err := cmd.Run(); err != nil {
		fmt.Printf("MP4Box processing note: %v\n", err)
	}
	if (strings.Contains(track.PreID, "pl.") || strings.Contains(track.PreID, "ra.")) && Config.DlAlbumcoverForPlaylist {
...
func mvDownloader(adamID string, saveDir string, token string, storefront string, mediaUserToken string, track *task.Track) error {
	...
	tagsString := strings.Join(tags, ":")
	mp4boxExec := getMP4BoxExec()
	muxCmd := exec.Command(mp4boxExec, "-itags", tagsString, "-quiet", "-add", vidPath, "-add", audPath, "-keep-utc", "-new", mvOutPath)
...
func getMP4BoxExec() string {
	if p, err := exec.LookPath("MP4Box"); err == nil {
		return p
	}
	binMp4Box := filepath.Join("bin", "mp4box.exe")
	if _, err := os.Stat(binMp4Box); err == nil {
		return binMp4Box
	}
	if _, err := os.Stat("mp4box.exe"); err == nil {
		return ".\\mp4box.exe"
	}
	if _, err := os.Stat("MP4Box.exe"); err == nil {
		return ".\\MP4Box.exe"
